...
class API_Client {
    ...
    const OPTION_USERNAME              = 'wdcs_api_username';
    const OPTION_PASSWORD              = 'wdcs_api_password';
    const OPTION_WSDL                  = 'wdcs_api_wsdl';
    const OPTION_FORCE_LOCATION        = 'wdcs_api_force_location';
    const OPTION_TIMEOUT               = 'wdcs_api_timeout';
    const OPTION_DEBUG                 = 'wdcs_api_debug';
    const OPTION_SOAP_VERSION          = 'wdcs_api_soap_version';

    const OPTION_LAST_RAW_JSON         = 'wdcs_last_raw_excerpt';
    const OPTION_LAST_RAW_XML          = 'wdcs_last_raw_xml';
    const OPTION_LAST_PRODUCTS_XML     = 'wdcs_last_products_xml';
    const OPTION_LAST_REQUEST_XML      = 'wdcs_last_request_xml';
    const OPTION_LAST_SOAP_FAULT       = 'wdcs_last_soap_fault';
    const OPTION_LAST_REQUEST_HEADERS  = 'wdcs_last_request_headers';
    const OPTION_LAST_RESPONSE_HEADERS = 'wdcs_last_response_headers';
    const OPTION_LAST_INFOCARDS_META   = 'wdcs_last_infocards_meta';

    private $wsdl;
    private $username;
    private $password;
    private $force_location;
    private $timeout;
    private $debug;
    private $soap_version;

    public function __construct() {
    $this->wsdl           = get_option( self::OPTION_WSDL, 'http://92.241.78.182:8080/artsw2022/ws/WebExchange.1cws?wsdl' );
    $this->username       = get_option( self::OPTION_USERNAME, '' );
    
    // Handle password with improved decryption (supports backward compatibility)
    $password_stored = get_option( self::OPTION_PASSWORD, '' );
    ...
    $this->force_location = get_option( self::OPTION_FORCE_LOCATION, '' );
    ...
    public function fetch_cards() {
        return $this->fetch_cards_remote();
    }

    public function fetch_products_raw() {
        // Apply security checks before API call
        do_action( 'wdcs_before_api_call' );
    ...
    public function fetch_cards_remote() {
    // Apply security checks before API call
    do_action( 'wdcs_before_api_call' );
    ...
    private function maybe_store_debug( \SoapClient $client, $response, $tag ) {
    ...
    private function capture_fault_artifacts( \SoapClient $client, \SoapFault $fault, $op, $start ) {
        if ( ! $this->debug ) return;
    ...
    private function capture_generic_exception_artifacts( \SoapClient $client, \Throwable $e, $op, $start ) {
        if ( ! $this->debug ) return;
    ...
    private function parse_information_cards( $response ) {
    ...
    private function normalize_discount( $val ) {
        $f = (float) $val;
        if ( $f < 0 ) $f = 0;
        return (int) round( $f );
    }
    ...
    private function excerpt( $text, $limit ) {
        $t = (string)$text;
        if ( strlen( $t ) > $limit ) {
